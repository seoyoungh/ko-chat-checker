...
2. input 말뭉치와 기존 trained corpus를 대조, 비표준어 찾아냄
   - 1) if 말뭉치 == 표기 OK:
      ...
      - 만약, 강한 규칙 적용이라면! (to solve 그게 ``모양``? 네가 ``조화``! 그게 ``모임``?)
...
* **May**
  - 표기 오류가 없는 문어체 + 대화체 외부 데이터셋 구축 ⭕️
  - 표기 오류가 있는 데이터, 없는 데이터 분리 ⭕️
    - 1차 분류: 대화체 데이터셋에 있으면 표기 오류 없다고 판단
    - 2차 분류: 문어체 데이터셋에 있으면 표기 오류 없다고 판단
      - Word2Vec pre-trained model 사용함
    - 3차 분류: -당, -잇-과 같은 자주 쓰이는 채팅체 수작업으로 재분류
  - 표기 오류가 있는 데이터 라벨링 작업 수행 💪🏻
    - 다음 맞춤법 검사기 활용
  - ``character level text similarty`` 계산 💪🏻
    - ``Edit Distance``로 찾은 교정 단어와 라벨과의 일치 정확도 계산
    - 우선은 우리의 correctly spelled dataset 이용해서 진행해보기
      - 정확도가 낮다면 수집한 외부 대화체 데이터 활용
      - 관련 없는 데이터셋 제거, 우리가 찾은 채팅체 단어 corpus에서 제거 필요
    - [빠른 검색을 위한 reference](https://lovit.github.io/nlp/2018/09/04/levenshtein_inverted_index/)
  - 모델 학습  
    - 음절로 분리해 학습 진행
      - e.g.) ``(ㅁ,ㅓ,_) (ㄱ,ㅓ,_) (ㅆ,ㅓ,_)``
    - 사용해볼 모델
      - CharCNN
      - FastText
      - ELMo
